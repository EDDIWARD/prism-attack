...
class MMLU:
    ...
    def load_data(self):
        ...
        # FIXED: Use .head() instead of .sample() for deterministic sampling
        self.selected_data = self.data.head(self.n_samples)
...
class Math:
    ...
    def filter_data(self):
        # FIXED: Use .head() instead of .sample() for deterministic sampling
        self.selected_data = self.data[self.data['level'] >= self.level_geq].head(self.n_samples)
...
class MedMCQA:
    ...
    def load_data(self):
        ...
        # FIXED: Use .head() instead of .sample() for deterministic sampling
        self.selected_data = self.filtered_data.head(self.n_samples)
...
class ProfessionalLaw:
    ...
    def load_data(self):
        self.data = pd.read_json(self.data_file, lines=True)
        # cop is 0-indexed (0=A,1=B,2=C,3=D), convert to 1-indexed for MedMCQA compatibility
        self.data['cop'] = self.data['cop'] + 1
        self.selected_data = self.data.head(self.n_samples)

    def __getitem__(self, idx):
        item = self.selected_data.iloc[idx].to_dict()
        if 'id' in item:
            item['original_index'] = item['id']
        return item
...
def get_dataset(dataset_name='mmlu', n_samples=50, data_dir='data', context=False):
    ...
    if dataset_name == 'mmlu':
        ...
        return ProfessionalLaw(dataset_name, n_samples, data_dir)
    else:
